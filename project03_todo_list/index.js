...
let toDoList = [];
for (let i = 1; i > 0; i++) {
    const opt = await input();
    if (opt === "Add_task") {
        const task = await getTask("Write task you want to add: ");
        ...
        const task = await getTask("Write task you want to remove: ");
        if (toDoList.includes(task)) {
        ...
        const task = await getTask("Write task you have completed: ");
        if (toDoList.includes(task)) {
            toDoList.indexOf(task);
